...
The plugin also adds the 🧀 favicon to every docs site as an inline SVG data URI, so a site needs no `public/favicon.svg`. Plain HTML pages import the same string as `cheeseFavicon` from the package root.
